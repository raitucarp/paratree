...
class Sentence
  implements IParaNode, ISentence, IParaNodeMethod<Sentence, Word> {
  ...
  next(onlyInParent?: boolean): Sentence | null {
    return nextNode<Paragraph, Sentence>(this.parent, this.index, onlyInParent);
  }

  prev(onlyInParent?: boolean): Sentence | null {
    return prevNode<Paragraph, Sentence>(this.parent, this.index, onlyInParent);
  }
